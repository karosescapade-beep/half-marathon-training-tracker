...
function renderWorkout(task) {
  ...
  const hasStrength = workout.includes("Barry") || workout.includes("F45") || workout.includes("OTF");
...
function getTaskWorkoutCredit(task, profile) {
  ...
  if (hasRun) credit += 1;
  if (hasStrength) credit += getChoiceWorkoutCredit(task.id, "Strength", getStrengthDefault(workout), profile);
  if (hasFlexibility) credit += getChoiceWorkoutCredit(task.id, "Flexibility", "Pilates", profile);
