...
const appBuildVersion = "v62";
const maxRunMiles = 13.1;
const remoteApplyGraceMs = 15000;
...
let lastDirectActionAt = 0;
let lastDirectActionKey = "";
const recentLocalEdits = new Map();
...
function canEditProfileContent(profile) {
  return Boolean(profile);
}

function shouldSyncProfile(profile) {
  return googleSyncEnabled && profile?.id !== "profile-you" && canEditProfileContent(profile);
}
...
function extractGoogleSheetWebAppUrl(text) {
  const matches = String(text || "").match(/https:\/\/script\.google\.com\/[^\s"'<>]+/gi) || [];
  const realUrls = matches.filter((url) => !url.includes("YOUR_DEPLOYMENT_ID"));
  return realUrls.at(-1)?.trim() || "";
}

async function loadGoogleSheetWebAppUrl() {
  setGoogleSheetWebAppUrl(googleSheetWebAppUrl);
}
...
function getPublicProfile(profile) {
  return {
    ...
    order: compactOrder(profile.order || {}),
    recovery: compactStringObject(profile.recovery || {}),
    choices: compactChoiceObject(profile.choices || {}),
    completed: compactCompleted(profile.completed || {}),
    runMileage: compactMileage(profile.runMileage || {}),
    longRunMileage: compactMileage(profile.longRunMileage || {}),
    theme: visualThemeOptions.includes(profile.theme) ? profile.theme : "anime",
  };
}

function markProfileLocallyEdited(profile = getActiveProfile()) {
  if (profile?.id) recentLocalEdits.set(profile.id, Date.now());
}

function hasRecentLocalEdit(profileId) {
  const editedAt = recentLocalEdits.get(profileId);
  return Boolean(editedAt && Date.now() - editedAt < remoteApplyGraceMs);
}
...
function remoteProfileMatchesPayload(remoteProfile, expectedProfile) {
  if (!remoteProfile || !expectedProfile || remoteProfile.id !== expectedProfile.id) return false;

  return (
    stablePayloadJson(remoteProfile.order) === stablePayloadJson(expectedProfile.order) &&
    stablePayloadJson(remoteProfile.choices) === stablePayloadJson(expectedProfile.choices) &&
    stablePayloadJson(remoteProfile.completed) === stablePayloadJson(expectedProfile.completed) &&
    stablePayloadJson(remoteProfile.runMileage) === stablePayloadJson(expectedProfile.runMileage)
  );
}

function compactCompleted(completed = {}) {
  return Object.fromEntries(Object.entries(completed).filter(([, value]) => value === true));
}

function compactStringObject(value = {}) {
  return Object.fromEntries(
    Object.entries(value).filter(([, entry]) => typeof entry === "string" && entry.trim() !== ""),
  );
}

function compactChoiceObject(value = {}) {
  return Object.fromEntries(
    Object.entries(value).filter(([, entry]) => {
      if (Array.isArray(entry)) return entry.every((choice) => typeof choice === "string");
      return typeof entry === "string" && entry.trim() !== "";
    }),
  );
}

function compactMileage(value = {}) {
  return Object.fromEntries(
    Object.entries(value).filter(([, entry]) => {
      const miles = Number(entry);
...
function compactOrder(order = {}) {
  return Object.fromEntries(
    Object.entries(order).filter(([weekIndex, taskIds]) => {
      const index = Number(weekIndex);
      const defaultOrder = defaultPlacements[index];
      return (
        Array.isArray(taskIds) &&
        Array.isArray(defaultOrder) &&
        taskIds.length === defaultOrder.length &&
        taskIds.some((taskId, taskIndex) => taskId !== defaultOrder[taskIndex])
      );
    }),
  );
}
...
function makeMutationUrl(payload) {
  ...
  return `${googleSheetWebAppUrl}?${params.toString()}`;
}

function hideManualSyncButton() {
  if (!manualSyncButton) return;
  const profile = getActiveProfile();
  const visible = googleSyncEnabled && shouldSyncProfile(profile);
  manualSyncButton.hidden = !visible;
  manualSyncButton.disabled = !visible;
  manualSyncButton.dataset.syncUrl = "";
  manualSyncPayload = visible
    ? {
        action: "upsert",
        ownerKey: browserOwnerKey,
...
function showManualSyncButton(payload) {
  if (!manualSyncButton || !googleSyncEnabled) return;
  manualSyncButton.hidden = false;
  manualSyncButton.dataset.syncUrl = "";
  manualSyncPayload = payload;
}
...
function submitToSheet(payload) {
  ...
  const syncUrl = makeMutationUrl(payload);
  const completePayload = {
    ...payload,
    build: appBuildVersion,
  };

  if (JSON.stringify(completePayload).length > 1200) {
    return submitToSheetFrame(completePayload, "_self");
  }

  return new Promise((resolve) => {
    const pixel = new Image();
    pixel.onload = () => resolve({ ok: true });
    pixel.onerror = () => resolve({ ok: true });
    pixel.src = syncUrl;
    window.setTimeout(() => {
      resolve({ ok: true });
    }, 1800);
...
function submitToSheetFrame(payload, target) {
  return new Promise((resolve) => {
    const frameName = target === "_blank" ? "_blank" : `sheetPostFrame_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const frame = target === "_blank" ? null : document.createElement("iframe");
    ...
    window.setTimeout(() => {
      ...
      resolve({ ok: true });
    }, target === "_blank" ? 500 : 2200);
  });
}

function openManualSyncTab(payload) {
  if (!payload || !googleSyncEnabled) return;
  submitToSheetFrame(
    {
      ...payload,
      build: appBuildVersion,
...
function applyRemoteProfiles(profiles) {
  ...
  profiles.forEach((remoteProfile) => {
    ...
    if (hasRecentLocalEdit(profile.id)) return;
...
async function syncProfileToSheet(profile = getActiveProfile()) {
  ...
  try {
    await submitToSheet(syncPayload);
    updateSyncStatus("Save sent to Google Sheet. Confirming...", "saving");

    try {
      const profiles = await fetchProfilesUntil((sheetProfiles) =>
        sheetProfiles.some((remoteProfile) => remoteProfileMatchesPayload(remoteProfile, syncPayload.profile)),
      );
      if (!profiles.some((remoteProfile) => remoteProfileMatchesPayload(remoteProfile, syncPayload.profile))) {
        throw new Error("Updated profile was not found in the Google Sheet yet.");
      }

      recentLocalEdits.delete(profileId);
      applyRemoteProfiles(profiles);
      showManualSyncButton(syncPayload);
      updateSyncStatus("Synced with Google Sheet", "success");
    } catch {
      showManualSyncButton(syncPayload);
      updateSyncStatus("Auto-sync blocked. Click Sync to Google.", "error");
    }
  } catch (error) {
    showManualSyncButton(syncPayload);
...
function getSelectableProfiles() {
  ...
  const namedProfiles = profiles.filter((profile) => profile.id !== "profile-you");
  return namedProfiles.length ? namedProfiles : profiles;
}

function ensureSelectableProfile() {
  const selectableProfiles = getSelectableProfiles();
  if (!selectableProfiles.length) return;
  if (selectableProfiles.some((profile) => profile.id === appState.activeProfileId)) return;

  const editableProfile = selectableProfiles.find(canEditProfileContent);
  appState.activeProfileId = (editableProfile || selectableProfiles[0]).id;
}
...
function applyDrop(list, event) {
  ...
  markProfileLocallyEdited(profile);
  saveState();
  dragState = null;
  renderWeeks();
  updateProgress();
...
function moveTaskWithinWeek(weekIndex, taskId, direction) {
  const profile = getActiveProfile();
  if (!canEditProfileContent(profile)) return;

  const placements = [...getWeekPlacements(weekIndex, profile)];
  const currentIndex = placements.indexOf(taskId);
  const nextIndex = currentIndex + direction;

  if (currentIndex === -1 || nextIndex < 0 || nextIndex >= placements.length) return;

  [placements[currentIndex], placements[nextIndex]] = [placements[nextIndex], placements[currentIndex]];
  profile.order[weekIndex] = placements;
  appState.activeWeekIndex = weekIndex;
  markProfileLocallyEdited(profile);
  saveState();
  renderWeeks();
  updateProgress();
  renderLeaderboard();
  requestAnimationFrame(() => scrollToWeek(weekIndex, "auto", false));
}

function createMoveButton(label, direction, disabled) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = direction < 0 ? "move-task-button move-task-up" : "move-task-button move-task-down";
  button.setAttribute("aria-label", label);
  button.dataset.directAction = "move-task";
  button.dataset.moveDirection = String(direction);
...
function getSelectedChoiceValues(taskId, type, defaultValue, profile = getActiveProfile()) {
  const key = `${taskId}-${type}`;
  const savedChoice = profile.choices?.[key];
  const legacyRecovery = type === "Flexibility" ? profile.recovery?.[taskId] : null;
  const fallbackChoice = savedChoice || legacyRecovery || defaultValue;
  const rawValues = Array.isArray(savedChoice) ? savedChoice : fallbackChoice ? [fallbackChoice] : [];

  return rawValues.filter((choice) => typeof choice === "string" && choice.trim() !== "");
}

function toggleChoiceValue(currentChoices, value) {
  if (value === "None") {
    return currentChoices.includes("None") ? [] : ["None"];
  }

  const withoutNone = currentChoices.filter((choice) => choice !== "None");
  return withoutNone.includes(value)
    ? withoutNone.filter((choice) => choice !== value)
    : [...withoutNone, value];
}

function setChoiceValue(taskId, type, value, defaultValue = "") {
  const profile = getActiveProfile();
  if (!canEditProfileContent(profile)) return;
  const key = `${taskId}-${type}`;
  const nextChoices = toggleChoiceValue(getSelectedChoiceValues(taskId, type, defaultValue, profile), value);
  profile.choices[key] = nextChoices;
  if (type === "Flexibility") profile.recovery[taskId] = nextChoices[0] || "";
  markProfileLocallyEdited(profile);
  saveState();
  renderWeeks();
  updateProgress();
...
function setTaskMileage(task, nextValue) {
  const profile = getActiveProfile();
  if (!canEditProfileContent(profile)) return;
  const parsedValue = Number(nextValue);
  const value = Math.max(0, Math.min(maxRunMiles, Number.isFinite(parsedValue) ? parsedValue : task.baseMiles));
  ...
  appState.activeWeekIndex = task.weekIndex;
  markProfileLocallyEdited(profile);
  saveState();
  renderWeeks();
  updateProgress();
...
function handleDirectAction(event) {
  ...
  const trigger = target?.closest?.("[data-direct-action]");
  if (!trigger || trigger.disabled) return;

  const actionKey = [
    trigger.dataset.directAction,
    trigger.dataset.taskId || "",
    trigger.dataset.choiceType || "",
    trigger.dataset.choiceValue || "",
    trigger.dataset.moveDirection || "",
    trigger.dataset.mileageDelta || "",
  ].join(":");
  const now = Date.now();

  if (actionKey === lastDirectActionKey && now - lastDirectActionAt < 450) {
    event.preventDefault();
    event.stopPropagation();
    return;
  }

  lastDirectActionAt = now;
  lastDirectActionKey = actionKey;
  event.preventDefault();
  event.stopPropagation();

  if (trigger.dataset.directAction === "move-task") {
    moveTaskWithinWeek(
      Number(trigger.dataset.weekIndex),
      trigger.dataset.taskId,
      Number(trigger.dataset.moveDirection),
    );
  }

  if (trigger.dataset.directAction === "choice") {
    setChoiceValue(
      trigger.dataset.taskId,
      trigger.dataset.choiceType,
      trigger.dataset.choiceValue,
      trigger.dataset.choiceDefault || "",
    );
  }
}

function bindDirectAction(button) {
  button.addEventListener("pointerup", handleDirectAction);
  button.addEventListener("touchend", handleDirectAction, { passive: false });
  button.addEventListener("click", handleDirectAction);
}
...
function getTheme(workout) {
  ...
  if (workout.includes("Barry") || workout.includes("F45") || workout.includes("OTF")) return "Strength";
  return "Training";
}
...
function isSundayTrainingTask(task) {
  return task.originalDay?.startsWith("Sun") && getTheme(task.workout) !== "Celebrate";
}

function makeChoiceSelect(taskId, type, options, defaultValue) {
  const profile = getActiveProfile();
  const editable = canEditProfileContent(profile);
  ...
  const selectedChoices = getSelectedChoiceValues(taskId, type, defaultValue, profile);

  options.forEach((optionLabel) => {
    const option = document.createElement("option");
    option.value = optionLabel;
    option.textContent = optionLabel;
    option.selected = selectedChoices.includes(optionLabel);
    select.append(option);
  });

  select.addEventListener("change", () => {
    setChoiceValue(taskId, type, select.value, defaultValue);
  });
  ...
  options.forEach((optionLabel) => {
    ...
    button.dataset.directAction = "choice";
    button.dataset.taskId = taskId;
    button.dataset.choiceType = type;
    button.dataset.choiceValue = optionLabel;
    button.dataset.choiceDefault = defaultValue;
    button.setAttribute("aria-pressed", selectedChoices.includes(optionLabel) ? "true" : "false");
    button.disabled = !editable;
    if (selectedChoices.includes(optionLabel)) button.classList.add("active");
    bindDirectAction(button);
...
function makeThemeLine(theme, content) {
...
function getTaskMiles(task, profile = getActiveProfile()) {
  ...
  if (Number.isFinite(saved) && saved >= 0) return Math.min(saved, maxRunMiles);
...
function makeRunMileageControl(task) {
  ...
  const currentMiles = getTaskMiles(task, profile);

  const input = document.createElement("input");
  input.type = "range";
  input.min = "0";
  input.max = String(maxRunMiles);
  input.step = "0.1";
  input.value = formatMiles(currentMiles);
  input.setAttribute("aria-label", "Run mileage");
  input.disabled = !editable;

  const value = document.createElement("span");
  value.className = "mileage-value";
  value.textContent = `${formatMiles(currentMiles)} mi`;
  ...
  input.addEventListener("input", () => {
    value.textContent = `${formatMiles(Number(input.value))} mi`;
  });

  input.addEventListener("change", () => {
    setTaskMileage(task, input.value);
  });

  wrap.append(input, value, detail);
  return wrap;
}
...
function renderWorkout(task) {
  ...
  const isSundayTraining = isSundayTrainingTask(task);
  const hasStrength = isSundayTraining || workout.includes("Barry") || workout.includes("F45") || workout.includes("OTF");
  const hasFlexibility = isSundayTraining || workout.includes("Pilates") || workout.toLowerCase().includes("flexibility");
  ...
  if (hasStrength) {
    const strengthOptions = hasTwoExercises ? ["Barry's", "F45", "OTF", "None"] : ["Barry's", "F45", "OTF"];
    const strength = makeChoiceSelect(id, "Strength", strengthOptions, isSundayTraining ? "None" : getStrengthDefault(workout));
  ...
  if (hasFlexibility) {
    const flexibilityOptions = hasTwoExercises
      ...
      : ["Pilates", "Barre", "Yoga"];
    const flexibility = makeChoiceSelect(id, "Flexibility", flexibilityOptions, isSundayTraining ? "None" : "Pilates");
...
function getChoiceWorkoutCredit(taskId, type, defaultValue, profile) {
  return getSelectedChoiceValues(taskId, type, defaultValue, profile).filter((choice) => choice !== "None").length;
}

function getTaskWorkoutCredit(task, profile) {
  const workout = task.workout;
  const isSundayTraining = isSundayTrainingTask(task);
  const hasStrength = isSundayTraining || workout.includes("Barry") || workout.includes("F45") || workout.includes("OTF");
  const hasFlexibility = isSundayTraining || workout.includes("Pilates") || workout.toLowerCase().includes("flexibility");
  const hasRun = task.isRun || workout.includes("miles");
  ...
  if (hasRun) credit += 1;
  if (hasStrength) credit += getChoiceWorkoutCredit(task.id, "Strength", isSundayTraining ? "None" : getStrengthDefault(workout), profile);
  if (hasFlexibility) credit += getChoiceWorkoutCredit(task.id, "Flexibility", isSundayTraining ? "None" : "Pilates", profile);

  return credit || (!hasRun && !hasStrength && !hasFlexibility ? 1 : 0);
}
...
function getWeekCompletedMiles(weekIndex, profile = getActiveProfile()) {
  return [...taskById.values()]
    .filter((task) => task.weekIndex === weekIndex && task.isRun && profile.completed[task.id])
    .reduce((total, task) => total + getTaskMiles(task, profile), 0);
}

function getCompletedRunMiles(profile) {
  return [...taskById.values()]
    .filter((task) => task.isRun && profile.completed[task.id])
    .reduce((total, task) => total + getTaskMiles(task, profile), 0);
}

function getWeekBadgeMarkup(week, weekIndex, profile) {
  if (week.badge === "Race") return "<strong>Race week</strong>";
  const completedMiles = getWeekCompletedMiles(weekIndex, profile);
  return `<strong>${formatMiles(completedMiles)} miles</strong>`;
}
...
function renderWeeks() {
  ...
  weeks.forEach((week, weekIndex) => {
    ...
    const badgeMarkup = getWeekBadgeMarkup(week, weekIndex, profile);
    header.innerHTML = `
      <div>
        <h3>${week.title}</h3>
        <p>${week.range} - ${week.focus}</p>
      </div>
      <span class="week-badge">${badgeMarkup}</span>
    `;
    ...
    getOrderedTasks(weekIndex, profile).forEach((taskData, taskIndex) => {
      ...
      const moveUpButton = createMoveButton("Move day earlier this week", -1, !editable || taskIndex === 0);
      const moveDownButton = createMoveButton("Move day later this week", 1, !editable || taskIndex === week.tasks.length - 1);
      [moveUpButton, moveDownButton].forEach((button) => {
        button.dataset.weekIndex = String(weekIndex);
        button.dataset.taskId = taskData.id;
        bindDirectAction(button);
      });

      moveControls.append(moveUpButton, moveDownButton);

      task.append(grip, checkbox, copy, moveControls);
      task.classList.toggle("completed", checkbox.checked);
      list.append(task);
    });
...
function getProfileStats(profile) {
  ...
  const workoutCredits = completedTasks.reduce((sum, task) => sum + getTaskWorkoutCredit(task, profile), 0);
  const miles = getCompletedRunMiles(profile);
  const percent = total ? Math.round((completed / total) * 100) : 0;
  const score = Math.round((workoutCredits + miles) * 10) / 10;

  return { completed, total, percent, miles, score, workoutCredits };
}
...
function updateProgress() {
  ...
  profileStatus.textContent = editable
    ? `${profile.name}'s report is active.`
    : `${profile.name}'s report is view-only on this device.`;
...
function renderProfileSelect() {
  ...
  getSelectableProfiles().forEach((profile) => {
    const option = document.createElement("option");
...
function openProfileFromLeaderboard(profileId) {
...
function renderLeaderboard() {
  ...
  rows.forEach(({ profile, stats }, index) => {
    ...
    row.setAttribute("aria-label", `View ${profile.name}'s progress`);
    if (profile.id === appState.activeProfileId) row.classList.add("active-athlete");
    row.addEventListener("click", () => openProfileFromLeaderboard(profile.id));
    row.addEventListener("keydown", (event) => {
      if (event.key !== "Enter" && event.key !== " ") return;
      event.preventDefault();
      openProfileFromLeaderboard(profile.id);
    });
    ...
    identity.className = "leaderboard-person";
    const name = document.createElement("h3");
    name.textContent = profile.name;
    const completed = document.createElement("p");
    completed.textContent = `${stats.completed}/${stats.total} workouts completed`;
    ...
    progress.append(percent, progressTrack);

    const miles = document.createElement("span");
    miles.className = "leaderboard-metric";
    miles.innerHTML = `<small>Run miles</small><strong>${formatMiles(stats.miles)}</strong>`;

    const score = document.createElement("span");
    score.className = "leaderboard-metric leaderboard-score";
    score.innerHTML = `<small>Score</small><strong>${formatMiles(stats.score)}</strong>`;
    const metrics = document.createElement("div");
    metrics.className = "leaderboard-metrics";
    metrics.append(miles, score);

    row.append(rank, identity, progress, metrics);
...
document.addEventListener("touchend", handleDirectAction, { capture: true, passive: false });
document.addEventListener("click", handleDirectAction, true);

manualSyncButton?.addEventListener("click", () => {
  if (!manualSyncPayload) return;
  openManualSyncTab(manualSyncPayload);
  updateSyncStatus("Sync tab opened. Refresh the Sheet after ok:true.", "saving");
});
...
deleteProfileButton.addEventListener("click", () => {
  ...
  const fallbackProfile =
    Object.values(appState.profiles).find((candidate) => candidate.id !== "profile-you") ||
    appState.profiles["profile-you"] ||
    Object.values(appState.profiles)[0];
