...
function getWeekBadgeMarkup(week, weekIndex, profile) {
  if (week.badge === "Race") return "<strong>Race</strong><small>week</small>";
  const completedMiles = getWeekCompletedMiles(weekIndex, profile);
  const plannedMiles = getWeekPlannedMiles(weekIndex, profile);
  return `<strong>${formatMiles(completedMiles)} mi</strong><small>of ${formatMiles(plannedMiles)} planned</small>`;
}
...
function renderLeaderboard() {
  ...
  rows.forEach(({ profile, stats }, index) => {
    ...
    miles.innerHTML = `<small>Miles</small><strong>${formatMiles(stats.miles)}</strong>`;
