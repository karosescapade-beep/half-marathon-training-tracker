...
function showManualSyncButton(payload) {
  ...
  manualSyncButton.dataset.syncUrl = makeMutationUrl(payload);
}
...
function submitToSheet(payload) {
  ...
  const directUrl = makeMutationUrl(payload);

  if (directUrl.length < 7000) {
    return loadJsonp(directUrl).catch(() => submitToSheetFrame(payloadJson, directUrl));
  }

  return submitToSheetFrame(payloadJson, directUrl);
}

function submitToSheetFrame(payloadJson, directUrl) {
...
manualSyncButton?.addEventListener("click", () => {
  const syncUrl = manualSyncButton.dataset.syncUrl;
  if (!syncUrl) return;
