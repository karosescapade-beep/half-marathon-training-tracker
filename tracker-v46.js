...
function getPublicProfile(profile) {
  return {
    ...
    choices: compactStringObject(profile.choices || {}),
...
function ensureSelectableProfile() {
  ...
  const editableProfile = selectableProfiles.find(isProfileEditable);
...
function setChoiceValue(taskId, type, value) {
  const profile = getActiveProfile();
  if (!isProfileEditable(profile)) return;
  const key = `${taskId}-${type}`;
  profile.choices[key] = value;
  if (type === "Flexibility") profile.recovery[taskId] = value;
  saveState();
  renderWeeks();
}
...
function handleDirectAction(event) {
  ...
  if (trigger.dataset.directAction === "choice") {
    setChoiceValue(trigger.dataset.taskId, trigger.dataset.choiceType, trigger.dataset.choiceValue);
  }
}
...
function makeChoiceSelect(taskId, type, options, defaultValue) {
  ...
  select.addEventListener("change", () => {
    setChoiceValue(taskId, type, select.value);
  });
  ...
  options.forEach((optionLabel) => {
    ...
    button.setAttribute("aria-pressed", selection === optionLabel ? "true" : "false");
    button.disabled = !editable;
    if (selection === optionLabel) button.classList.add("active");
