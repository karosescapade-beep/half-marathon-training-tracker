...
function makeChoiceSelect(taskId, type, options, defaultValue) {
  ...
  ["pointerdown", "touchstart", "mousedown", "click"].forEach((eventName) => {
    select.addEventListener(eventName, protectControlTap, { passive: true });
  });
...
function makeRunMileageControl(task) {
  ...
  ["pointerdown", "touchstart", "mousedown", "click"].forEach((eventName) => {
    input.addEventListener(eventName, protectControlTap, { passive: true });
  });
...
function renderWeeks() {
  ...
  weeks.forEach((week, weekIndex) => {
    ...
    getOrderedTasks(weekIndex, profile).forEach((taskData, taskIndex) => {
      ...
      task.draggable = editable && !shouldUseMobileMoveControls();
      ...
      task.addEventListener("dragstart", (event) => {
        if (!isProfileEditable(profile)) return;
        if (event.target.closest("input, select, button, textarea, label")) {
      ...
      moveUpButton.addEventListener("click", (event) => {
        event.stopPropagation();
        moveTaskWithinWeek(weekIndex, taskData.id, -1);
      });
